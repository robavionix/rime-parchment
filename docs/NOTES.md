...
## 4.2：九宫格中文标点通路 + 左侧标点列跳键盘问题

### 现象
九宫格最左侧标点列，点选一个标点上屏后，键盘立刻跳回 26 键。

### 定位
那一列不是按键，是元书内置组件：

```jsonnet
t9SymbolsCollection: { params: { type: 't9Symbols' } }   // 无任何可配参数
```

行为写在元书 App 内部：平时显示标点、有 preedit 时切换成拼音候选，
**选完一项返回 App 认定的「主键盘」**。皮肤只能声明放一个它，无法干预。
`reference/空山素影/jsonnet/Buttons/Layout9.libsonnet` 的定义与此一字不差，
说明不是本皮肤引入的。官方文档（中文九键页、皮肤页）均未提及此组件，无开关。

### 排除的方案
- **改元书主键盘为九宫格**：九宫格不跳了，26 键开始跳，方向对调而已。
- **换掉 t9SymbolsCollection**：那一列打字时是拼音候选（实机确认），
  换成自定义按键等于砍掉九宫格选音，代价远大于收益。

### 实机确认的关键事实
普通按键（`action: { symbol: '@' }`）上屏符号**不跳**。
即：跳键盘是 `t9Symbols` 组件自带的行为，不是「上屏符号」这个动作带来的。
...
1 键点按仍走 Rime punctuator（`1 @ . / : _ - #`），有 preedit 时仍是分词键，
原有行为一律未改，纯增量。

README 已写入「已知问题」章节，如实说明成因与元书侧的责任边界。

---

## 4.3：标点键位重排 + 左侧标点列跳转的真正解法

### 解法修正（推翻 4.2 的结论）
4.2 里写的「只能靠元书作者加开关」是错的。实机发现：
**在元书 App 的符号设置里删掉 `，` 和 `。` 两条，左侧标点列就不再跳键盘。**
其余符号本来就不跳。

即：跳转不是 `t9Symbols` 组件对所有条目的统一行为，而是与这两个具体条目
相关。机制未知（元书未公开该组件实现），但现象可稳定复现与规避。

`t9Symbols` 无参数、皮肤无法干预、上游定义一致——这几条事实不变，
变的只是「用户侧有解」这一点。README 已按此重写。

### 键位重排
| 键 | 手势 | 输出 |
|---|---|---|
| 标点键（原 @ 键，键名仍为 latinSymbolButton） | 点按 | `，` |
| | 上划 | `。` |
| | 长按 | `、：；“”（）— ~ / _ & # *` |
| 1 键 | 长按 | `@ ？ ！ … % £ $ ¥ + - × ÷ =` |
| | 下划 | `，` |

设计取向：逗号句号占中文标点用量绝大部分，给最快的两个手势（点按、上划）
直出；数学与货币符号集中在 1 键长按一字排开，免去切数字键盘。

1 键点按（Rime punctuator）、上划（数字 1）、打字时分词，三条原有行为未改。
键名 `latinSymbolButton` 保留不改，避免已编译样式名与用户覆盖失效。

---

# 5.0：废除 t9 方案，九宫格并入 rime_ice

## 现象

切 App 呼出键盘，布局是九宫格，但打出来是一串数字；
切到 26 键再切回九宫格就正常。从 4.x 一路带到 V9.9 都没治好。

## 两次误诊

**误诊一（4.6 / 4.7 / V9.8）：以为是 `ascii_mode` 跨会话残留。**
上游 `switches` 里 `ascii_mode` 确实没有 `reset`，开着的时候按键原样透传，
九宫格上就是数字——症状对得上。补了 `switches/@0/reset: 0`（两个补丁各打一遍，
因为 `.custom.yaml` 不跟着 `__include` 传递）。
结果：**停在中文模式仍然复现**。那条修复本身是对的，只是治的不是这个病。

**误诊二（V9.9）：以为默认方案没对齐。**
把 `t9` 排进 `schema_list` 第一位。
结果：**无效**。`schema_list[0]` 只决定全新部署时的初始方案，
覆盖不了元书的运行时记忆。

## 真实成因

**方案是记忆的，布局是固定的，两者各记各的。**

- 元书恢复的是**上次用过的 Rime 方案**
- 键盘布局由皮肤 `primaryKeyboard` **写死**

只要上次停在 26 键（哪怕打的是中文），记住的就是 `rime_ice`。
下次呼出，布局是九宫格、方案是 `rime_ice`——而 `rime_ice` 的
`speller/alphabet` 里没有数字，九宫格按键发的 `2`～`9` 不被当作输入码，
原样上屏。

切键盘能治，是因为切换键是 `combine`，带 `switchRimeSchema`。

**诊断法**：呼出键盘看空格键，它显示的就是当前方案名。
显示「雾凇拼音」而键盘是九宫格，即确诊。

## 为什么只能这么修

元书皮肤格式里**没有「键盘加载时绑定方案」的机制**。
布局 yaml 是纯样式 + 按键定义的扁平结构，`switchRimeSchema` 只存在于
按键动作里——也就是说，方案只能在**用户按下某个键之后**才能被纠正，
而不能在键盘出现的那一刻就对齐。

既然错配无法在发生后自动纠正，就只能让它**不可能发生**：删掉 `t9`,
两个键盘共用 `rime_ice`，元书记住哪个都是它。

## 为什么代价很小

`t9.schema.yaml` 第一行就是 `__include: rime_ice.schema.yaml:/`——
它本来就是 `rime_ice` 的派生，只多三样：8 条数字映射、`t9_processor`、
`prism: t9`。把前两样并进 `rime_ice`（第三样是编译产物，自然消失）即可。

反方向（统一到 `t9`）评估过，**代价大得多**：`t9` 的 engine 里没有
`melt_eng` / `cn_en` 翻译器，也没有 `autocap_filter` / `long_word_filter` /
`pin_cand_filter` / `v_filter`，`custom_phrase` 还是数字编码的另一份。
26 键会失去中英混打——那是本项目的立身之本。

## 并入的内容与两处坑

全部在 `src/patches/rime_ice.custom.yaml` 末尾，`T9-ONLY` 标记之间。
`build-rime.sh android` 用 `sed` 按标记剥离，安卓 / PC 拿到纯 26 键配置。

**坑一：`punctuator/half_shape` 必须逐键打补丁，不能整块重定义。**
`rime_ice` 的 `half_shape` 是 `__include: default:/punctuator/half_shape`，
整块覆盖会把全部中文标点一起干掉。旧 `t9.schema.yaml` 就是整块覆盖的，
它不在乎是因为九宫格标点走元书内置组件——26 键可不行。
故写成 `punctuator/half_shape/1`、`"punctuator/half_shape/@"` 这样的逐键路径
（`@` 和 `#` 要把整个 key 加引号，否则 YAML 解析出的 key 会带上引号字符）。

**坑二：数字映射必须排在模糊音与容错之后。**
`derive` 是叠加式的，排最后意味着上面产生的所有拼写（含 `wam`/`fanh`/`zhomg`
这些容错形式）都会一并被映射成数字，九宫格因此同样享有容错。
旧 `t9.custom.yaml` 里那条「模糊音必须在数字映射之前」的告诫，针对的是
上游把数字映射写死在 algebra 中间的情况，此处顺序相反，结论也相反。

## speller 要改两行，不是一行

`speller/alphabet` 决定哪些键算输入码，`speller/initials` 决定哪些键能**起头**。
数字只加进 alphabet 而没加进 initials 时，空串状态下按 `2` 不被认为能开始组词，
于是原样上屏 —— 九宫格永远只出数字，切键盘也救不回来（比不改还糟，
因为原来的错配至少切一下能好）。`t9.schema.yaml` 是两行都带数字的。

## 未验证项（实机才能定）

1. **建 prism 会不会爆。** `超级简拼 + 6 条模糊音 + 3 条容错 + 8 条数字映射`
   叠在 187 万条词库上，比此前任何一版都重。而「加规则导致部署失败、
   完全无法输入中文」在 4.x 已经发生过一次（见上文失败记录）。
   若失败，第一步是把 8 条数字映射移到模糊音**之前**，牺牲九宫格容错。
2. **`t9_processor` 会不会干扰 26 键。** 它是元书编译进 App 的黑盒，
   无源码可读。在旧 `t9` 方案里它排在 `ascii_composer` 之前，理论上只处理
   数字。若 26 键异常，删掉那一行即可——代价是九宫格预编辑区显示数字串
   而非拼音，候选仍然正确。
3. 上游 `others/iRime/` 下有一份把 `rime_ice` 直接改造成九宫格的补丁，
   用 `lua_filter@*t9_preedit` 解决预编辑显示。**本项目不能用**：
   那个 filter 无条件把所有候选的 preedit 改写成 comment，
   在双布局方案里会把 26 键的英文预编辑也一起改掉（打 `inter` 显示
   `international`）。它是为纯九宫格方案设计的。
