...
local pinyin9Component = import 'Components/Pinyin9.libsonnet';
local englishComponent = import 'Components/English.libsonnet';
...
local nameToComponent = {
  ...
  [if settings.dualKeyboard then 'english']: englishComponent,
   ...
   // 这几个组件只有在特定条件下才会被使用
  [if !settings.dualKeyboard && !std.startsWith(settings.keyboardLayout, '26') then 'temp26Key']: temp26Component,
